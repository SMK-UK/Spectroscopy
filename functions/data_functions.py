...
def search_paths(folders: list[str], 
    ...
    """
    Search a list of paths for include and exclude conditions.
    
    Parameters
    ----------
    folders : list
        List of folder names to search in.
    files : list
        List of file names.
    include : list, optional
        Keywords to include in paths.
    ...
    """
    # Convert files to a list of lists if there's only one folder
    if len(folders) == 1:
        files = [files] 
    ...
    for index, folder in enumerate(folders):
        ...
        for file in files[index]:
            ...
            if include:
                if not any(x in path for x in include):
                    continue
            if exclude:
                if any(y in path for y in exclude):
                    continue
            desired.append(path)
